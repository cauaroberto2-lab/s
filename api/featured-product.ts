...
const FEATURED_PRODUCT_KEY = 'pais-store:featured-product:v1';
...
function publicRecord(record: FeaturedProductRecord | null) {
  return record ? { featuredProductId: record.featuredProductId, updatedAt: record.updatedAt } : { featuredProductId: null, updatedAt: null };
}
...
export default async function handler(request: ApiRequest, response: ServerResponse) {
  ...
  try {
    ...
    if (!product) {
      sendJson(response, 422, { error: 'product_not_in_catalog', message: 'O produto escolhido não existe mais no catálogo sincronizado.' });
      return;
    }
