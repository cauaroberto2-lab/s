...
import type { ApiRequest } from '../server/admin-auth';

type Handler = (request: ApiRequest, response: ServerResponse) => Promise<void>;
...
async function run() {
  ...
  try {
    ...
    const unauthorizedSave = await fetch(`${api.origin}/api/featured-product`, {
    ...
    const product = catalog.products.find((item) => !item.archived);
    assert.ok(product, 'O catálogo de teste precisa ter um produto ativo.');
    ...
    const reloadedFromAnotherSession = await fetch(`${api.origin}/api/featured-product`);
    assert.equal(reloadedFromAnotherSession.status, 200);
    assert.equal((await reloadedFromAnotherSession.json() as { featuredProductId: string }).featuredProductId, product.id);
    ...
    console.log('Featured product API: authentication, server validation and durable reload passed.');
  } finally {
    api.server.close();
    redis.server.close();
  }
